--[[
\\
  Linoria | Mobile Supported | Made by iksuwu
  Original: https://github.com/RizzSf/UiLibrary/blob/main/Linoria%20Lib/docs.md
  -----------------------------------------------------------------------------
  V1.3
//
]]
...
getgenv().Toggles = Toggles; -- if you load infinite yeild after you executed any script with LimixWareLib it will just break the whole UI lib :/ (thats why I added getgenv().LimixWare)
...
function Library:CreateWindow(...)
	...
	function Library:Toggle()
		...
		if Toggled then
			...
			if Library.ShowCustomCursor and Drawing then
				...
				RunService:BindToRenderStep("LimixWareCursor", Enum.RenderPriority.Camera.Value - 1, function()
					...
					if not (Toggled and ScreenGui.Parent and Library.ShowCustomCursor) then
						...
						RunService:UnbindFromRenderStep("LimixWareCursor")
					end
				end)
			end
		end;
	...
	if Library.IsMobile then
		...
		LockUIButton.MouseButton1Down:Connect(function()
			LockUISS = not LockUISS
			LockUIButton.Text = LockUISS and "Unlock UI" or "Lock UI"
			Library.CantDragForced = LockUISS
		end)
	end;
